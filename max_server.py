...
def handler_function(address, *args):
    ...
    if args and args[0] == "generate":
        ...
        result = requests.get(f"{SERVER_URL}/generate", params={"genres": ','.join(genres)})
        if not result.ok:
            ...
            f_path = '/tmp/' + result_file_name
            with open(f_path,'wb') as f:
                f.write(decoded_content)
            client.send_message("read", str(f_path))
...
server = osc_server.ThreadingOSCUDPServer(("0.0.0.0", 8989), dispatcher)
